...
MQTTCtx_t *mqtt_ctx_create(const char *host, const char *client_id,
                           const char *topic_name, MqttQoS qos,
                           word16 keep_alive_sec, word32 cmd_timeout_ms,
                           MqttMsgCb msg_cb) {
    int rc = -1;
    ...
    rc = MqttClient_Init(&ctx->client, &ctx->net, msg_cb, ctx->tx_buf,
                         MAX_BUFFER_SIZE, ctx->rx_buf, MAX_BUFFER_SIZE,
                         ctx->cmd_timeout_ms);
...
int mqtt_ctx_connect(MQTTCtx_t *ctx, int conn_timeout) {
    int rc = MqttClient_NetConnect(&ctx->client, ctx->host, ctx->port,
                                   conn_timeout, ctx->use_tls, NULL);
    ...
    PRINTF("MQTT Connect: Proto (%s), %s (%d)",
           MqttClient_GetProtocolVersionString(&ctx->client),
    ...
    PRINTF("MQTT Connect Ack: Return Code %u, Session Present %d",
           ctx->connect.ack.return_code,
           (ctx->connect.ack.flags & MQTT_CONNECT_ACK_FLAG_SESSION_PRESENT)
               ? 1
               : 0);
    return rc;
}
...
int mqtt_subscribe(MQTTCtx_t *ctx, const char *topic_name, MqttQoS qos) {
    ...
    if (ctx->topics_count < 10) {
        ...
        PRINTF("already subscribed %d topics, no more room for another topic.",
               ctx->topics_count);
        return -1;
    }
...
int mqtt_start(MQTTCtx_t *ctx) {
    rt_thread_t tid =
        rt_thread_create("mqtt", matt_thread, ctx, MQTT_THREAD_STACK, 5, 5);
    if (tid) {
